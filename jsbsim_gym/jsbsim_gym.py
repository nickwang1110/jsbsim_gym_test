...
class JSBSimEnv(gym.Env):
    ...
    def __init__(self, root='.',render_modes=None):
        super().__init__()

        # Set observation and action space format
        self.observation_space = gym.spaces.Box(STATE_LOW, STATE_HIGH, (15,),dtype='float64')
    ...
    def step(self, action):
        ...
        return np.hstack([self.state, self.goal]), reward, done, False, {}
    ...
    def reset(self, seed=None,options=None):
        ...
        return np.hstack([self.state, self.goal]),info
...
if __name__ == "__main__":
    ...
    env = JSBSimEnv()
    JSBSimEnv.reset = lambda self, **kwargs: self.env.reset(**kwargs)
    #env.reset()
    env.render()
    for _ in range(300):
